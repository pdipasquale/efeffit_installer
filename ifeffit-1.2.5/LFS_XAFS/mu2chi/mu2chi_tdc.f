...
c START
c--------------------------------------------------------------------
       i = ifeffit('read_data(data/tdc.dat, group=mudata, type=xmu)')
       i = ifeffit('mudata.energy=mudata.1')
       i = ifeffit('mudata.xmu=mudata.2')
...
c      sets mudata.pre to be the pre-edge subtracted xmu
      i = ifeffit('pre_edge(mudata.energy,mudata.xmu,e0=8335.7)')
c--------------------------------------------------------------------
c END PRE_EDGE CALC
c--------------------------------------------------------------------
...
c     sets mudata.bkg
     ...
     &     kweight=2,e0=e0=8335.7, nknots=i_nknots,
     &        rbkg=i_rbkg)')
c--------------------------------------------------------------------
...
c      i = ifeffit('mudata.e0 = ones(npts(mudata.k)) * e0')
      ...
      i = ifeffit('write_data(file=data/chi_xmu.dat,
     &                        mudata.energy,mudata.xmu,mudata.bkg,
     &                        mudata.k, first_energy)')
