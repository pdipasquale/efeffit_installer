...
c     i = ifeffit('set $1.chik = $1.chi * $1.k^kweight')
      ...
      i = ifeffit('guess(alpha=-0.004)')
      i = ifeffit('guess(sigma2=0.00267)')
...
c       i = ifeffit('guess(s02=0.85)')
      ...
      do j=1,num_paths
         if (j.lt.10) then
              write(ifeffit_arg,'(A,I1,A)')'guess(alpha=0.)'
c              write(ifeffit_arg,'(A,I1,A)')'guess(s02=0.65)'
              i = ifeffit(ifeffit_arg)
     ...
     &                                   ', delr=reff*alpha*da',j,')'
            i = ifeffit(ifeffit_arg)
              ...
              write(ifeffit_arg,'(A,I2,A)')'guess(alpha=0.)'
              i = ifeffit(ifeffit_arg)
              write(ifeffit_arg,'(A,I2,A)')'guess(s02=0.65)'
              i = ifeffit(ifeffit_arg)
...
c      i = ifeffit('s02_norm_correction=0.00023')
      ...
      do j=1,num_paths
         if (j.lt.10) then
c            write(ifeffit_arg,'(A,I1,A)')'guess(s02=0.92)'
c            write(ifeffit_arg,'(A,I1,A)')'guess(s02',j,'=0.92)'
c            i = ifeffit(ifeffit_arg)
            write(ifeffit_arg,'(A,I1,A,I1,A)')'path(index=',j,
     &                                   ',feff=feff/feff000',j,'.dat,'
            i = ifeffit(ifeffit_arg)
