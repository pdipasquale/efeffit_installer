...
c interface for general  function fitting
       ...
       double precision  getsca, xfmin, xfmax, toler, toldef
       parameter (toldef = 1.d-5)
       ...
       external   genfun, get_array, istrln, getsca, nofx
...
c  interpret any and all keyword/value pairs for setting options
       ...
       do 100 i = 1, nkeys
          ...
          elseif (keys(i).eq.'restraint')  then
             ...
             call echo(' *** minimize: unknown key: '//keys(i)(1:k))
          end if
 100   continue
c
       ...
       if (nfdata.le.0) then
          call echo( ' minimize : no function to minimize?')
          return
       end if
       cfmin_pre = name1
       cfmin_arr = fitarr
...
c  and look at setx1 / setx2 for user-limited fit range
       if ((x_arr.ne.undef)) then 
          ...
          if (ixa.ne.0) then
             ...
             call echo( '            for setting fit bounds')
          end if
       end if
...
cc            if ((lminfo.ne.0).and.(lminfo.ne.4).and.(lminfo.ne.5)) then
 ...
 480            continue
          ...
          call echo(' minimize:  no variables defined.')
       end if
