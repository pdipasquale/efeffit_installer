...
c  show values for ifeffit
     ...
     $         '@commands, @macros, @colors, @paths, @memory, @limits'
       ...
       if (int(getsca('&sync_level')).ge.2)  call iff_sync
       nkeys = mkeys
       do 5 i = 1, nkeys
          keys(i) = blank
 5     continue 
...
c check (in this order): commands, macros, scalars
 ...
 970         continue 
          ...
          if (.not.found) then
             ...
             call echo( blank//class(1:iex)//blank//
     $            s(:ier)//'  not found')
          end if
          go to 200
       end
...
c
 ...
 10    continue 
       ...
       if (i.gt.macmax) then
          tmpstr = str
          k      = istrln( tmpstr )
          call echo( ' macro '// tmpstr(1:k) //' not found')
          return
       endif
...
c read feff files if necessary
       if (.not. lffred(jfeff)) then
          call fefinp
          if (int(getsca('&sync_level')).ge.3)  call iff_sync
       endif          
...
c      path degeneracy
       ...
       call echo (messg)
c
c if there is no path label, construct one here from reff, degen, path
 25    format('reff=', f8.4,', degen=', f5.1)
       ilen = istrln(pthlab(inpath))
       if (ilen.le.1) then 
          write(tmpstr,25) refpth(jfeff), pdeg
          ilen = istrln(tmpstr)
          tmpstr= tmpstr(1:ilen)//', path: '//at_symbol(izpth(0,jfeff))
          ilen = istrln(tmpstr)
          do 30 i = 1,  nlgpth(jfeff)-1
             tmpstr= tmpstr(1:ilen)//'->'//at_symbol(izpth(i,jfeff))
             ilen  = istrln(tmpstr)
 30       continue 
          tmpstr= tmpstr(1:ilen)//'->'//at_symbol(izpth(0,jfeff))
          ilen  = istrln(tmpstr)
          pthlab(inpath) = tmpstr(1:ilen)
       endif
       write(messg,'(4x,2a)') 'id     = ', pthlab(inpath)(1:ilen)
       call echo (messg)
...
cc       print*, ' xtmp ', xtmp
       ...
       call write_double_param('reff  ', 6, refpth(jfeff))
       call write_double_param('degen ', 6, xtmp)
       call write_double_param('n*s02 ', 6, param(jfps02))
