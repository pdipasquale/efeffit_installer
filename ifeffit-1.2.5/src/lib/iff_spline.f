...
c get default values for pre-edge parameters from current scalar values
       splstf = 1.d-4
cc       splstf = getsca('spl_stiff')
       e0    = getsca('e0')
       rbkg  = getsca('rbkg')
       r1stx = 2.d0
       toler = getsca('toler')
       xkmin = getsca('kmin_spl')
       xkmax = getsca('kmax_spl')
       xkw   = getsca('kweight_spl')
       dk1   = getsca('dk1_spl')
       dk2   = getsca('dk2_spl')
       pre1  = getsca('pre1')
       pre2  = getsca('pre2')
...
c     $         '|',values(i)(1:20),'|'
          ...
          elseif (keys(i)(1:5).eq.'k_std') then
             ...
             call echo(' *** spline: unknown keyword " '//messg)
          end if
 100   continue
...
cc       print*, ' spline : name1 = ', name1(1:40)
       ...
       if (name1.eq.undef) then
          call echo( ' spline: can''t determine group name')
          return             
       endif
       call fixnam(name1,1)
       call lower(name1)
...
cc       print*, 'spline ', jen, jxmu, jkst, jcst
       if (jen.le.0) then
          call echo( ' spline: no energy array?')
          return
       elseif(jxmu.le.0) then
          call echo( ' spline: no xmu array?')
          return
       elseif(npts_e.ne.npts_x) then
          call echo( ' spline: energy and xmu are different length?')
          return
       end if
c
...
cc          print*, ' find_e0 in pre_edge: ', find_e0, e0
     ...
     $         //xmuarr(1:lx)//tmpstr(1:l1)
          l1 = istrln(outstr)
          write(tmps2,'(a,f13.4,a,f13.4,a,f13.4,a,f13.4)') ', pre1=',
     $         pre1,',pre2=',pre2,',norm1=',enor1,',norm2=',enor2
          lx = istrln(tmps2)
...
c k and chi arrays for standard --
       if (do_std) then
          if (jkst.le.0) then
             call echo( ' spline: could not find k_std array?')
             return
          end if
          if (jcst.le.0) then
             call echo( ' spline: could not find chi_std array?')
             return
          end if
       end if
