...
c            "pref"."suff" form.
       ...
       integer maxcol, maxtit, maxsout
       parameter (maxcol = 64, maxtit=64, maxsout=64)
       integer       mpts, iout(maxcol), lun, is, ilen, idot, lx
       ...
       character*256 file, pref,  type, sout, defkey(2)
       character*256 arrout(maxcol), scaout(maxsout)
...
c fill lists of strings, scalars, arrays according to name
             if (isvnam(keys(i),0)) then
                ncol = ncol + 1
                if (ncol.le.maxcol)  arrout(ncol) = keys(i)
cc                print*, 'arr: ', keys(i)(1:20)
             elseif (isvnam(keys(i),1)) then
                nsca = nsca + 1
                if (nsca.le.maxsout)  scaout(nsca) = keys(i)
cc                print*, 'sca: ', scaout(nsca)(1:20)
...
cc                print*, 'tit: ', titout(ntit)(1:20)
                ...
                call echo(' *** write_data: unknown variable " '//messg)
             endif
          end if
 100   continue 
c
...
c
       ...
       if ((ier.lt.0).or.(iex.lt.0)) then
          call echo(' *** write_data: error opening file ')
          return
       end if
...
cc       print*, ' nsca, ntit, ncol = ', nsca, ntit, ncol
       ...
       if (ncol.eq.0) then
          call echo( ' *** write_data:  no data to write out!')
          return
       end if
       do 180 i = 1, ncol
          ...
          do 160 j = 1, nmatch
             ...
             if (k.le.0) then 
                ...
                jcol = jcol + 1
                if (jcol.le.maxcol) then
                   iout(jcol) = k
                   mpts = min(mpts, narray(k))
                else
                   call echo(' *** write_data: too many arrays'//
     $                  ' requested for write')
                   write(tmpstr,'(5x,a,i3)') 'current limit is ',maxcol
                   call echo(tmpstr)
                   return
                endif
             end if
...
cc                print*, ' TITLE ', i,  titout(i)(1:30)
 ...
 250         continue 
          ...
          nsca = min(nsca, maxsout)
          do 280 i = 1, nsca
             xval = getsca(scaout(i))
...
c      write label line:  suffixes only!!!
 ...
 350      continue 
       ...
       if (jcol.ne.ncol) then
          call echo(' *** write_data: wrong number of vectors??')
          return
       end if
c write vectors
       do 500 i = 1, mpts
