...
c
       ...
       double precision  e0tst, qmntst, qmxtst,  xtmp, qtmp, e0t
       double precision  tmp, getsca
       parameter (small  = 1.d-10, toldef = 1.d-5)
       external splfun, nofx, sumsqr, getsca
       save
c
       nnorm  = 3
...
c    knots are points at which the spline has extra freedom.
       if ( (korder.lt.3).or.(nsplin.lt.korder) ) then
          call echo(' spline error: not enough data to create spline.')
          return
       end if
...
cc
          if (nr.ne.nr1st)  then
             call echo(' spline error: fitfft is broken' )
             stop
          end if
       end if
c
...
c
       if ((loop.le.5).and.eevary.and.(ntest.ne.nsplin) ) then
          ...
          call echo(' the # of knots in the spline changed'//
