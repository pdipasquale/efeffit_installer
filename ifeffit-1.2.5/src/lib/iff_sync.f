...
c
       ...
       integer  nplast, nlast, npnext_max
...
c
       ...
       if (npnext.gt.npnext_max) then
          ...
          call echo(' ***          please clean up arrays ')
       endif
 ...
 830   continue
cc       print*, ' iff_sync #  arrays: #empty ', nxarr
...
c Step #0  put fitting variables first:  simple swap in place
       if (iprint.ge.17)  call echo (' lining up variables')
       itmp = 0
       do 1100 iv = 1, nxscal
...
cc          print*, 'SYNC:array iv ',iv,arrnam(iv)(1:30)
cc          print*, 'SYNC: ', nparr(iv), narray(iv),
cc     $         icdarr(1,iv),icdarr(2,iv)
          if (lswitch) then
             itmp = itmp + 1
c if itmp .ne. iv, swap these two places (names, value, icodes)
             if (itmp.ne.iv) then
...
cc                   print*, ' swapping arrays ', iv, itmp
cc                   print*, itmp, narray(itmp), arrnam(itmp)(1:20)
cc                   print*, iv,   narray(iv),   arrnam(iv)(1:20)
...
c        if (iprint.ge.9) then 
cc       call echo( ' synvar:  nvarys, nconst, nxscal, ncarr, nxarr: ')
cc       print*,  nvarys, nconst, nxscal, ncarr, nxarr
c        end if
       do 500 iu =  1,2
          do 10 i = 1, maxpts
             tmparr(i) = 0
