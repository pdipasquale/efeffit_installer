...
c note the mixed precision: pgplot uses single precision
       ...
       double precision prange_min
       parameter (prange_min = 1.d-8, scale_key=16.0)
...
c
 ...
 70          continue
          ...
          elseif (keys(i).eq.'key_x') then
             ier = iff_eval_re( values(i), xkey_loc)
...
c
          ...
          elseif (k.ge.1) then
             ...
             call echo(' ** plot -- unknown keyword "'//messg)
          end if
 99       continue
 100   continue
c
c
     ...
     $         (xfunc.eq.undef).and.(yfunc.eq.undef)) then
             call echo(' ** plot warning: nothing to plot')
             return
...
c
       ...
       if (nplpts(nplot).le.1) then
          if (nx1.le.1) then
             call echo(' ** plot error: no data in x array ')
          elseif (ny1.le.1) then
             call echo(' ** plot error: no data in y array ')
          end if
          return
       end if
c
...
cc       print*, 'PLOT newdev? ', newdev
       ...
       if (icurdev .le. 0) then
          call echo(' ** plot error: cannot open device:'//plotf)
          return
       end if
...
c plot the graph.
       ...
       do 700 i = 1, nplot
          ...
          idum = isty(i)
cc
cc legend setting currently hardwired in!
          if (istrln(pltkey(i)).ge.1) then
             show_key = .true.
             ikey     = ikey+1
             ynkey    = (scale_key - 0.7 - ikey) / scale_key
             yykey(1) = ynkey * tlim(4) + (1.0 - ynkey) * tlim(3)
             yykey(2) = yykey(1)
             xxkey(1) = tlim(1) + (tlim(2) - tlim(1)) * (xkey_loc-0.050)
             xxkey(2) = tlim(1) + (tlim(2) - tlim(1)) * (xkey_loc-0.025)
             xnkey    = tlim(1) + (tlim(2) - tlim(1)) * (xkey_loc)
             ynkey    = yykey(1)-abs(tlim(4)-tlim(3))/(4*scale_key)
          endif
