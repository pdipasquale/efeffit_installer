...
c  save session to a  PAD save file
       ...
       if (int(getsca('&sync_level')).ge.1)  call iff_sync
       file  = 'ifeffit.sav'
...
c  interpret any and all keyword/value pairs for setting options
       ...
       do 100 i = 1, nkeys
          ...
          elseif (keys(i).eq.'strings_only')  then
             ...
             call echo(' *** save: unknown keyword " '//messg)
          end if
 100   continue 
       call undels(file)
       iex  = 0
       ...
       if ((ier.lt.0).or.(iex.lt.0)) then
          call echo(' *** save: error opening file ')
          return
       end if
...
cc       print*, ' iff_restore : ', file(1:40)
       ...
       do 100 i = 1, nkeys
          ...
          if (keys(i).eq.'file')  then
             ...
             messg = keys(i)(1:k)//' " will be ignored'
             call echo(' *** restore: unknown keyword " '//messg)
          end if
 100   continue 
c 
c note that the sync_level is stored here (otherwise we'd "restore" it!)
       i_sync = int(getsca('&sync_level'))
...
c arrays      
       ...
       return
 1010  continue 
       call echo(' *** restore: not a valid save file')
       return
       end
