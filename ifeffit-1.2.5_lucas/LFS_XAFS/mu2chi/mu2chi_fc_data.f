...
c START
c--------------------------------------------------------------------
      i = ifeffit('read_data(data/fc.dat, group=mudata, type=xmu)')
...
c      gms_interp(kpoints,my_k,my_chik,my_dchik,
c     &                    kpoints_int,interp_k,interp_chik,interp_dchik)
...
c      i = ifeffit(
c     &      'set mudata.dchi=linterp(mudata.gk,mudata.gdchi,mudata.k)')


      i = ifeffit('mudata.k    = mudata.gk')
      i = ifeffit('mudata.chi  = mudata.gchi')
      i = ifeffit('mudata.dchi = 0.1*mudata.gdchi')
