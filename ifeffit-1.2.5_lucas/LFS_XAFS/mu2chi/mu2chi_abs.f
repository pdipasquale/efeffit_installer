...
c     NEEDED FOR NON-INTERPOLATION
      ...
      real*8 my_energy(1000),my_chik(1000),my_k(1000)
      real*8 my_pre(1000),my_bkg(1000)
      real*8 my_dchik(1000),my_dxmu(1000)
ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
...
c      include '/home/lucasfs/ifeffit/share/ifeffit/config/ifeffit.inc'
      include 'ifeffit.inc'
...
c START
c--------------------------------------------------------------------
      i = ifeffit('read_data(data/sqc.dat, group=mudata, type=xmu)')
...
c      i = ifeffit('show e0, edge_step, pre1, pre2, norm1, norm2')
c      i = ifeffit('show pre_slope, pre_offset')
...
c--------------------------------------------------------------------
c my.y = interp(old.x,old.y,my.x)      
c      coded - Tauhid

      i = ifeffit( 'gms_interp(kpoints,my_k,my_chik,my_dchik,
     &            kpoints_int,interp_k,interp_chik,interp_dchik)')

c      i = iffputarr('mudata.k',    kpoints_int, interp_k)
c      i = iffputarr('mudata.chi',  kpoints_int, interp_chi)
c      i = iffputarr('mudata.dchi', kpoints_int, interp_dchi)

c      i = ifeffit(
c     &      'set mudata.chi=splint(mudata.gk,mudata.gchi,mudata.k)')
c       i = ifeffit('y = linterp(mudata.k,mudata.gdchi,interp_k)')
      
c      i = ifeffit('mudata.k    = mudata.gk')
c      i = ifeffit('mudata.chi  = mudata.gchi')

c      i = ifeffit('mudata.e0 = ones(npts(mudata.k)) * e0')
 
      i = ifeffit('mudata.k    = interp_k')
      i = ifeffit('mudata.dchi=linterp(mudata.gk,
     &            mudata.gdchi,mudata.k)')
      i = ifeffit('mudata.chi  = interp_chik')
c      i = ifeffit('mudata.dchi = y')

      i = ifeffit('mudata.e0 = ones(npts(mudata.k)) * e0')
      

c      i = ifeffit('mudata.dchi = linterp(mudata.gk,mudata.gdchi,interp_k)')
      call print_echo

      i = ifeffit('write_data(file=data/chi_k.dat,$title*,
     &                        mudata.k,mudata.chi,mudata.dchi,
     &                        mudata.e0)')

c      call plot_mu_v_energy
c      call plot_pre_edgea        
c      call plot_pre_suba
c      call plot_spline_new
c      call plot_my_chi
c      call plot_interp1
c      call plot_interp2
c      call plot_interpe
c      call plot_chi_vs_k_expt
...
c      i = ifeffit('title=\' [\\gm/\\gr] vs E\',xlabel=\'E (eV)\',')
c      i = ifeffit('ylabel=\' [\\gm/\\gr] (cm\\u2\\d/g) \',')
      i = ifeffit('xmin=6900,xmax=8000)')
...
c     Plot the pre-edge function function:
      ...
      i = ifeffit('style=solid, color=blue,')
c      i = ifeffit('title=\' [\\gm/\\gr]\\dedge\\u vs E\',')
c      i = ifeffit('xlabel=\'E (eV)\',')
c      i = ifeffit('ylabel=\' [\\gm/\\gr]\\dedge\\u(cm\\u2\\d/g) \')')
...
c      i = ifeffit('title=\' \\gx(k) vs k\',ylabel=\'\\gx(k)\',')
c      i = ifeffit('xlabel=\' k (\\A\\u-1\\d) \')')
...
c      i = ifeffit('title=\' \\gx(k) vs k\',')
c     i = ifeffit('ylabel=\' \\gx(k)\')')
...
c      i = ifeffit('title=\' d\\gx(k) vs k\',')
c      i = ifeffit('ylabel=\' d\\gx(k) \')')
