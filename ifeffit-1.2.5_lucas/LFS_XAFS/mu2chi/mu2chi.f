...
c START
c--------------------------------------------------------------------
      i = ifeffit('read_data(data/PFc15mM.dat, group=mudata, type=xmu)')
...
c      i = iffgetsca('e0',e0)
     ...
     &        my_bkg(j)
         ...
         my_dchik(j-first_energy)=0.5*my_dxmu(j)/my_bkg(j)
         j=j+1
      end do
...
c      coded - Tauhid
      ...
      i = ifeffit( 'gms_interp(kpoints,my_k,my_chik,mudata.gdchi,kpoints_int,interp_k,
     &            interp_chik,interp_dchik)')
...
c     &      'set mudata.chi=splint(mudata.gk,mudata.gchi,mudata.k)')
       i = ifeffit('dchi=linterp(mudata.gk,mudata.gdchi,mudata.k)')
c       i = ifeffit('interp_dchik=rebin(mudata.gk,mudata.gdchi,mudata.k)')
...
c      i = ifeffit('mudata.e0 = ones(npts(mudata.k)) * e0')
      ...
      i = ifeffit('write_data(file=data/chi_v_k.dat,$title*,
     &                        mudata.k,mudata.chi,mudata.dchi,
     &                        mudata.e0)')

      call plot_mu_v_energy
