...
c     i = ifeffit('set $1.chik = $1.chi * $1.k^kweight')
      ...
      i = ifeffit('guess(e0_cor=-5.27354)')
      i = ifeffit('guess(alpha=0.00)')
      i = ifeffit('guess(ss2=0.00267)')
      i = ifeffit('guess(s02=0.92)')
      
c  ------------ setting up restraint for S02 -----------------

      i = ifeffit ('set scale = 2000')
      i = ifeffit ('set s02a = 0.85')
...
c       i = ifeffit('guess(s02=0.85)')
     ...
     &                         ', sigma2=max(ss2, 0.001))'
            i = ifeffit(ifeffit_arg)       
            
            write(ifeffit_arg,'(A,I1,A)')'path(',j,
     &                              ', s02=max(0.7, min(s02, 1.2)))'
