...
c  GUESS / SET / DEF
       if (int(getsca('&sync_level')).ge.6)  call iff_sync
       isgues =  0
       isset  =  0
...
c    is this a text variable?
       if (istext) then
          if (.not.isvnam(tmpnam,3)) then
             ik = istrln(tmpnam)
             call echo(' Invalid text string name: '//
     $            tmpnam(1:ik) )
             go to 100
          end if
          if (tmpnam.eq.'$&build') then
             call echo(' can''t reset value of $&build')
             go to 100
          endif
...
c    check for immutable values, like pi:
       elseif (tmpnam.eq.'pi') then
          call echo(' can''t reset value of pi')
          go to 100
       elseif (tmpnam.eq.'etok') then
          call echo(' can''t reset value of etok')
          go to 100
...
c   first, check that it's not a macro or command name
          do 280 i = 1 , mckeys
             if (ckeys(i).eq.tmpnam) then 
                ik = istrln(tmpnam)
                call echo(' Invalid variable name: '//
...
c     this is a scalar
             ...
             if (idot.ne.0) then
                ...
                call echo('   '//tmpnam(1:k)//' = '//formula(1:ik))
                return
             end if
