...
c path list:
          elseif (values(i).eq.undef) then
             ...
             call echo(' *** feffit: unknown key: '//keys(i)(1:k))
          end if
 100   continue 
c===================================================================
...
c    set path list
       ...
       if (jchi.le.0) then
          call echo( ' feffit: no chi(k) data array?')
          return
       end if
...
c fit k-grid: chi(1) = chi(k=0).
 ...
 130         continue
             ...
             call echo( ' feffit: bad k-data array?')
             return
          endif
       else
          nkpts = jchi
...
c set k-window 
     ...
     $         qwindo(1,idata))
          ...
          if (jwin.le.0) then
             call echo(' feffit: no window array? looking for '
     $            //winarr)
             return
          end if
          if (xk_arr.ne.undef) then
...
c
 ...
 625      format (a,f8.3,a,f8.3,a)
       ...
       if (nvarys.gt.nfit(idata)) then
          call echo(' *** feffit: fewer fit data points '//
...
cc          if ((lminfo.ne.0).and.(lminfo.ne.4).and.(lminfo.ne.5)) then
 ...
 880            continue
          ...
          call echo(' feffit:  no variables defined.')
       endif
c 
