...
c error handling
 1000  continue 
       if (ierr.eq.ilog) then
          call echo(mtherr//'-- log(x) needs  x > 0')
       elseif (ierr.eq.ilog10) then
          call echo(mtherr//'-- log10(x) needs  x > 0')
       elseif (ierr.eq.isqrt) then
          call echo(mtherr//'-- sqrt(x) needs x >= 0')
       elseif (ierr.eq.iasin) then
          call echo(mtherr//'-- asin(x) needs (-1< x <1)')
       elseif (ierr.eq.iacos) then
          call echo(mtherr//'-- acos(x) needs (-1< x <1)')
       elseif (ierr.eq.idiv) then
          call echo(mtherr//'-- divide by 0 ')
       elseif (ierr.eq.iy2x) then
          call echo(mtherr//'-- invalid exponentiation ')
       elseif (ierr.eq.jdebye) then
          call echo(mtherr//' using "debye" function ')
          call echo(' *     could not find a path index to use!')
       elseif (ierr.eq.jeins) then
          call echo(mtherr//' using "eins" function')
          call echo(' *     could not find a path index to use!')
       elseif (ierr.eq.1) then
          call echo(death//'exceeded stack size')
       elseif (ierr.eq.2) then
          call echo(death//'too many objects')
       elseif (ierr.eq.5) then
          call echo(death//'mvpts .gt. maxpts')
       elseif (ierr.eq.9999) then
          call echo(death//' unknown operation')
       else
          call echo(death//' i am truly confused')
       end if
       if (ierr.gt.0) stop
